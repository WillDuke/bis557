...
##2. Implementation of a ridge regression function that handles collinear variables. 

The function `ridge_regression` in this package returns a list of $\beta$ coefficients while handling collinearity. 
...
The function `cv_ridge_regression` in this package returns a list containing a tibble detailing the RMSEs, standard error of the means, and CI bounds; a plot of the lambda values tested and their corresponding RMSEs; as well as the lambda with the lowest RMSE, `lambda_min`, and the lambda 1 standard error higher, `lambda_1se`.
...
The following code returns the condition number of a $40 \times 25$ matrix of random values by taking the ratio of the max and min singular values. Simulating the error rate for the OLS beta estimate reveals that when the condition number is low, the error in estimating beta tends to be low as well.
...
As you can see below, adding a $\lambda$ of 0.13 reduces the condition number to less than a fifth of its original value.
...
By simulating the error of the estimate calculated directly with the lambda term, we can see that there is a corresponding reduction in the variance. By introducing some bias, we can reduce the statistical error of our estimate.
